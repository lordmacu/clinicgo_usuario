<template>
  <div class="home" style="background-color:rgba(250,250,250,1); min-height:100vh;">
       <a rel="noopener" href="https://api.whatsapp.com/send?phone=56976081986&text=¡Hola%21%20Vengo%20de%20https://clinicgo.cl" class="float" target="_blank">
         <i class="fa fa-whatsapp my-float"></i>
      </a>
    <TopNavD  />
<div style="min-height:Calc(100vh - 570px);">
   ...
   <div class="bg" style="width:100vw; margin-top:-0px; position:absolute; background-size: cover; background-repeat: no-repeat; height:Calc(100vh - 170px); min-height:700px; z-index:0; top:0px; width:100%; background:linear-gradient(311deg, rgba(0,55,60,1) 0%, rgba(3,107,117,1) 100%);">
      -->
   <div class="bg" style="width:100vw; margin-top:-0px; position:absolute; background-size: cover; background-repeat: no-repeat; height:Calc(100vh - 170px); min-height:700px; z-index:0; top:0px; width:100%; background: linear-gradient(311deg, rgba(2,92,122,1) 0%, rgba(2,134,178,1) 100%);">
      <div style="position:relative; height:100%; max-width:1500px;  margin:0 auto;">
      <div class="persona-g">
      <div :style="{'background-image':'url(' + require('@/assets/personas/2.png') + ') '}" style="position:absolute; bottom:-1px; width: 100%; height:540px; background-position: right;  background-repeat: no-repeat;" > 
      
      </div>
      <!--<img style="position:absolute; bottom:-1px; right:-330px; " src="@/assets/personas/2.png" alt="">-->
      <div style="position:absolute; color:white; bottom:30px; right:370px; font-weight:700; font-size:18px;"> Gabriela, tomadora de muestra</div>
      </div>
      </div>
   </div>
   <div style=" top:0px; left: 0; right: 0; margin-left: auto; margin-right: auto; max-width:800px; ">
 

      <div style="position:relative; margin-top:22vh ">
         <div class="h2container2" style="padding: 0px 15px;">
            <div class="h2container">
               <h2 style="margin-bottom:30px;" class="titulo">Tomamos tu examen donde quieras y te enviamos los resultados en menos de 24 hrs.</h2>
            </div>
         </div>
         <barra-busqueda v-if="window.width >= 1000" />
    ...
    <div :style="{'height':(examenes_agregados.length*33 +220)+'px'}" v-if="examenes_agregados.length > 0" class="container" 
    style=" margin-top:65px; width:800px; max-width:100%;  margin-left: auto; margin-right: auto;   left: 0;  right: 0;   opacity:0.94; box-shadow: rgba(99, 99, 99, 0.2) 0px 2px 8px 0px; min-height:200px;  border:1px solid rgba(200,200,200,1); z-index:1; background:white; transform:translateY(-10px); border-radius:25px;">
     

<div class="" style="position:relative; padding:10px 0px 0px 0px; margin:0px;">
  <div style="">
<div style="padding: 0px; width:100%;">
   <h4>Resumen Orden</h4>
        <table style="width:100%; white-space: nowrap; ">
          <tr>
            <td style="width:30px; width:5%;"></td>
            <td style=" font-weight:600; width:85%;"></td>            
            <td style="font-weight:600; width:10%;"></td>
            <td style="width:100px; text-align:right; font-weight:600;  width:10%;"></td>
          </tr>
          ...
          <tr v-for="e in examenes_agregados">
            ...
            <td style="text-align:left;  overflow: hidden;max-width: 0; text-overflow: ellipsis;">{{e.nombre}}</td>
            <td style="text-align:left;"><b-icon style="cursor:pointer;" v-on:click="cambiarCantidad(e, 'restar')" icon="dash-circle"/> {{e.cantidad}} <b-icon style="cursor:pointer;" icon="plus-circle" v-on:click="cambiarCantidad(e, 'sumar')"/></td>
            <td style="text-align:right;">
               ${{Intl.NumberFormat('de-DE').format(e.precio*e.cantidad)}}</td>
          </tr>
        ...
        <table style=" margin-top:10px;">
         ...
         <tr style="">            
            <td style="text-align:right;  ">Total exámenes (reembolsable):</td>
            <td style="text-align:right;">
               ${{Intl.NumberFormat('de-DE').format(examenes_agregados.reduce((x, y)=>{return x+y.precio*y.cantidad}, 0))}}</td>
          </tr>
         <tr style="">            
            <td style="text-align:right; vertical-align:top; ">Toma de muestra (c/u):</td>
            <td style="text-align:right;">
              <div style="text-decoration:line-through;"> <span v-if="n_personas>1">${{Intl.NumberFormat('de-DE').format((costo_servicio+costo_servicio_pendiente))}}</span></div>  <span style="color:rgb(108, 193, 252);">${{  Intl.NumberFormat('de-DE').format(Math.round((costo_servicio+(n_personas*costo_servicio_pendiente))/n_personas)) }}</span></td>
          </tr>

          
          <tr style="font-weight:600; ">

            <td style="text-align:right; font-weight:600;">Total a pagar:</td>
            <td style="text-align:right;">${{Intl.NumberFormat('de-DE').format(examenes_agregados.reduce((x, y)=>{return x+y.precio*y.cantidad}, 0) + costo_servicio+(n_personas*costo_servicio_pendiente))}}</td>
          </tr>
      

        </table>
        <button  v-on:click="checkout();" style="color:white; float:right; margin-right:0px; margin-left:auto;" class="agendar-btn" @mouseenter="btnhover = true" @mouseleave="btnhover = false"  :style="{'background': comuna != -1 && examenes_agregados.length > 0 ? 'rgb(255, 200, 9)':'rgb(253,227,140)'}">Ir a agendar <b-icon style="" icon="arrow-right-circle"/></button>
    ...
    <div v-else style="height:323px;">

    </div>
</div>
<div class="spacer" style="height:Calc(-100vh + 720px); max-height:170px; min-height:120px;">
  ...
  <div id="comofunciona" >
    ...
    <div class="row">
      <div class="col-lg-4 comofunciona" data-aos="fade-up" data-aos-delay="0" style="position:relative;">

         <img style="width:100%; height:200px;" src="@/assets/svg/1.svg" alt="">

        <h3 style="margin-top:10px;">Busca y Agenda</h3>
        
        <p style="margin-top:20px;"><strong>Agenda</strong> la toma de uno o más <strong>exámenes</strong>, 100% online.</p>
       
      </div><!-- /.col-lg-4 -->

      <div data-aos="fade-up" data-aos-delay="50" class="col-lg-4 comofunciona">
      <img style="max-width:100%; max-height:200px;" src="@/assets/svg/4.svg" alt="">
        <h3 style="margin-top:10px;">Visita</h3>
        <p style="margin-top:20px;">Te <strong>tomamos la muestra</strong> en tu <strong>casa u oficina</strong>, en el horario agendado.</p>
      </div><!-- /.col-lg-4 -->
      <div data-aos="fade-up" data-aos-delay="100" class="col-lg-4">
      <img style="max-width:100%; max-height:200px;" src="@/assets/svg/5.svg" alt="">
        <h3 style="margin-top:10px;">Resultados</h3>
        <p style="margin-top:20px;">Dentro de <strong>máximo 24 horas</strong>, recibirás los <strong>resultados</strong> en tu correo electrónico o Whatsapp.</p>
      </div><!-- /.col-lg-4 -->
    </div><!-- /.row -->
        <!--
...
-->
  ...
  <div>
   <div class="rego-cont" style="position:relative;  background: rgba(20, 134, 231, 1); ">
<div class="botellitas" style="background-repeat: no-repeat; background-position: center; background-position: 200px 10px; " :style="{'background-image': 'url(' + require('@/assets/icon/lab2.svg') + ')'}">
   <!-- :style="{'background-image': 'url(' + require('@/assets/icon/lab.svg') + ')'}"-->
    ...
    <div  class="row featurette">
      ...
      <div data-aos="fade-right"  class="col-md-7 rego">
        ...
        <h2  class="featurette-heading" style="color:white;" > Trabajamos con Laboratorio Regonesi </h2> 
        
        
        <p class="lead" style="color:white;"> Con 90 años de trayectoria analizando muestras de laboratorio, Regonesi se encuentra ubicado en la comuna de Vitacura, y está certificado por el Ministerio de Salud de Chile.</p>
         <a href="https://www.laboratorioregonesi.cl/" class="rg-btn">Ir a Regonesi</a>
      </div>
      <div class="col-md-5">
         <div style="background:white; width:220px; height:220px; border-radius:120px; border:15px solid rgb(46, 151, 237); vertical-align:middle; margin:0 auto;">
            <img style="margin-top:20px;" src="@/assets/regonesi.png" alt="">
         </div>
      </div>
    </div>
    </div>
...
<div style="margin-top:0px;" >
   <div id="comunas">
      ...
      <div style="padding-bottom:90px; background-color:white;">
<div data-aos="fade-up" data-aos-delay="0">
   <h2 style="padding-top:120px;"  >Comunas en las que funcionamos</h2>
   <div class="divider"></div>
</div>

<div data-aos="zoom-in" data-aos-delay="0">
<div style="margin-top:70px;">
Actualmente estamos en <span v-for="c, v in comunas_usadas">{{v == comunas_usadas.length-1 ? 'y '+c.Nombre : c.Nombre+', '}}</span>.
</div>
...
<svg
  ...
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Padre Hurtado" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Maipu" :style="{'fill': comunas_usadas.some(item => item.id == 48) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Cerrillos" :style="{'fill': comunas_usadas.some(item => item.id == 54) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Estacion Central" :style="{'fill': comunas_usadas.some(item => item.id == 38) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
  ...
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Santiago" :style="{'fill': comunas_usadas.some(item => item.id == 37) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
  ...
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Recoleta" :style="{'fill': comunas_usadas.some(item => item.id == 44) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Independencia" :style="{'fill': comunas_usadas.some(item => item.id == 45) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Quilicura" :style="{'fill': comunas_usadas.some(item => item.id == 41) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Conchali" :style="{'fill': comunas_usadas.some(item => item.id == 42) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="San Bernardo" :style="{'fill': comunas_usadas.some(item => item.id == 59) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
  ...
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Pedro Aguirre Cerda" :style="{'fill': comunas_usadas.some(item => item.id == 56) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="San Miguel" :style="{'fill': comunas_usadas.some(item => item.id == 60) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
  ...
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="San Joaquin" :style="{'fill': comunas_usadas.some(item => item.id == 61) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
  ...
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Providencia" :style="{'fill': comunas_usadas.some(item => item.id == 47) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Ñuñoa" :style="{'fill': comunas_usadas.some(item => item.id == 50) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="La Reina" :style="{'fill': comunas_usadas.some(item => item.id == 49) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Macul" :style="{'fill': comunas_usadas.some(item => item.id == 115) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Puente Alto" :style="{'fill': comunas_usadas.some(item => item.id == 58) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
  ...
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="La Florida" :style="{'fill': comunas_usadas.some(item => item.id == 46) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Peñalolen" :style="{'fill': comunas_usadas.some(item => item.id == 53) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Las Condes" :style="{'fill': comunas_usadas.some(item => item.id == 39) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Vitacura" :style="{'fill': comunas_usadas.some(item => item.id == 51) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Lo Barnechea" :style="{'fill': comunas_usadas.some(item => item.id == 52) ? 'rgb(20, 134, 231)' : 'grey'}" />
  <path
     ...
     style="fill:grey;fill-opacity:1;fill-rule:evenodd;stroke:black;stroke-width:0.2;stroke-linecap:butt;stroke-linejoin:miter;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
     id="Huechuraba" :style="{'fill': comunas_usadas.some(item => item.id == 40) ? 'rgb(20, 134, 231)' : 'grey'}" />
</svg>
</div>
</div>
</div>
...
export default {
  ...
  metaInfo(){
     return{
        ...
        meta:[
           { name: 'description', content: 'Tomamos tu examen donde quieras y te enviamos los resultados en menos de 24 hrs.' },
        ]
     }
  },
  created() {
  ...
  methods: {
    ...mapActions(['clickearComunaAction', 'clickearExamenAction', 'cambiarCantidadAction', 'cambiarCantidadPersonasAction', 'quitarExamenAction']),
      handleResize() {
...
.cursivo{
   font-size:50px; font-style:italic; font-family: Gabriola, cursive; font-weight:100;
}
