<template>
    ...
    <div class="main-search">
      ...
      <div @mouseenter="hovering_exam = true" @mouseleave="hovering_exam = false" v-on:click="focusInputExam()" :style="{'width': focused_exam || focused_prev || focused_comuna ? '50%':'60%', '-webkit-box-shadow': focused_exam ? '2px 3px 25px -1px rgba(0,0,0,0.31)': 'none', 'background':focused_exam ? 'white': ''}" class="main-search-exam dont-close-exam">
        <div>
        <div  class="main-search-title dont-close-exam">Examen <span style="border:1px solid rgb(0,55,60); color: rgb(0,55,60); padding:0px 4px; border-radius:10px;" v-if="examenes_agregados.length > 0"> {{examenes_agregados.length}} Agregados <b-icon icon="check"/></span></div>
        ...
        <div :style="{'display':hovering_exam || focused_exam || hovering_comuna || focused_comuna ? 'none':'', 'border-left': focused_exam || focused_prev || focused_comuna ? '1px solid rgba(195,195,195,0)':'1px solid rgba(195,195,195,1)'} "  class="vl-exam"></div>
      </div>




      <div @mouseenter="hovering_comuna = true" @mouseleave="hovering_comuna = false" v-on:click="focusInputComuna($event)" :style="{'width': focused_exam || focused_prev || focused_comuna ? 'Calc(50% - 5px)':'Calc(40% - 5px)','-webkit-box-shadow': focused_comuna ? '2px 3px 25px -1px rgba(0,0,0,0.31)': 'none', 'background':focused_comuna ? 'white': ''}" class="main-search-comuna">
     
        <div class="main-search-title">Comuna <span style="border:1px solid rgb(0,55,60); color: rgb(0,55,60); padding:0px 4px; border-radius:10px;" v-if="comuna != -1"> <!-- <b-icon icon="check"/> --> {{comunas.filter(x=>{return x.id==comuna})[0].Nombre}} <b-icon icon="check"/></span> </div>
        <input style="font-size:16px;" class="dont-close-comuna data-hj-allow" v-on:keyup.enter="onEnterComuna()" v-model="comuna_buscado" :style="{'background':focused_comuna ? 'white': ''}" ref="comuna" @focus="focusInputComuna(); comuna_buscado = ''" type="text" :placeholder="comuna == -1 ? '¿En qué comuna?': comunas.filter(x=>{return x.id==comuna})[0].Nombre">
        ...
        :style="{'width': focused_exam || focused_prev || focused_comuna ? '120px':'50px',
        ...
        :class="{'glow': comuna != -1 && examenes_agregados.length > 0}"
                 >
                 <!--'#3D9970':'#FF4136' -->
                 <b-icon class="dont-close-btn dont-close-exam dont-close-prev dont-close-comuna" style="position:absolute; left:15px; top:15px;" icon="calendar"/><div class="dont-close-btn dont-close-exam dont-close-prev dont-close-comuna" :style="{'color': focused_exam || focused_prev || focused_comuna ? 'rgba(255,255,255,1)':'rgba(255,255,255,0)'}" style="margin-left:20px; display:inline; transition-delay:0.3s;">{{focused_exam || focused_prev || focused_comuna ? '  Agendar':''}}</div>
        </button>
    ...
    <div v-if="focused_exam"  style=" ertical-align:middle;" class="search-exam-popup dont-close-exam">
     ...
     <div class="dont-close-exam" v-else>
       <div class="alert alert-info" style="background-color:rgb(20, 134, 231, 0.2) !important; position:absolute; top:10px; font-style: italic; margin-left:10px; font-size:14px; padding:2px; margin-right:10px; margin-bottom:0px; width:Calc(100% - 20px);">
         ** Haz <strong> click en un examen </strong>para agregarlo. <img style="height:23px; width:23px; transform:rotate(-55deg);" src="@/assets/icon/tap.svg" alt="">
         </div>
       <ul style="padding-top:18px;">
         <li class="dont-close-exam " style="height:42px; position:relative;" v-on:click="onClickExam(l.item); $gtag.event('barrabusqueda-examen', {'event_category': 'busqueda', 'event_label': l.item.nombre, 'value': 1} );" v-for="l in resultado_fuse_examen.slice(0, 10)" >
          <!--
           ...
           -->
           <div class="dont-close-exam" style="display:inline-block; margin-left:10px; width:20px; transform:translateY(-5px);"> <b-icon style="height:18px; width:18px; color:rgb(20, 134, 231);" :icon="examenes_agregados.some(e => e.id == l.item.id) ? 'check': 'bag-plus'"/> </div>
           <div class="dont-close-exam" style="display:inline-block; overflow: hidden;  text-overflow: ellipsis; max-width:430px;  white-space: nowrap;">{{l.item.nombre}}</div>
           <div class="dont-close-exam" style="position:absolute; border:2px solid rgb(20, 134, 231); color:rgb(20, 134, 231); padding:4px; border-radius:10px; right:20px; top:8px;">
    ...
    <div v-if="focused_comuna"  style=" ertical-align:middle;" class="search-comuna-popup">
     ...
     <b-icon icon="search" style="height:40px; width:80px; padding-left:20px; padding-right:20px;" /> <div class="dont-close-comuna" style="height:60px; margin-left;40px; display:inline-block; vertical-align:middle;">Escribe para buscar tu comuna</div>
      ...
      <div v-else>
               <div class="alert alert-info" style="background-color:rgb(20, 134, 231, 0.2) !important; position:absolute; top:10px; font-style: italic; margin-left:10px; font-size:14px; padding:2px; margin-right:10px; margin-bottom:0px; width:Calc(100% - 20px);">
         ** Haz <strong> click en una comuna</strong> para seleccionarla. <img style="height:23px; width:23px; transform:rotate(-55deg);" src="@/assets/icon/tap.svg" alt="">
         </div>
       <ul style="padding-top:18px;">
...
-->
    <div class="tags-container">
      ...
      <div v-on:click="agregarExamenAction(examenes.filter(x=>{return x.id == 244})[0])" class="tag">Perfil Lipídico</div>
