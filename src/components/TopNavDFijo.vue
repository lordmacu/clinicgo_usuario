<template>
<div style="font-size: 14px; position:absolute; top:0px; width:100%; z-index:9999; border-bottom:1px solid rgb(240,240,240);">
...
<div v-if="examenes_agregados.length>0" style="position:absolute; top:29px; right:177px; color:white; background:rgb(108, 193, 252); width:20px; height:20px; font-size:14px; border-radius:10px;">{{examenes_agregados.reduce((x, y) => {return x + y.cantidad },0)}}</div>
<router-link v-if="!isLoggedIn" tag="a" to="/login"  class="loginbtn" href="">
   ...
   <router-link to="/PanelUsuario" tag="div" style="cursor:pointer; margin-top:30px; outline:none; margin-right:20px; color:black;"  class="loginbtn" v-if="isLoggedIn"><i class="fa fa-user" style="font-size:16px;  margin-right:5px;" aria-hidden="true"/>  {{usuario_datos.nombre}}</router-link>
  ...
  <div class="logo"> <router-link  style="cursor:pointer;" tag="div" :to="{name:'Home'}" ><img src="@/assets/logo3.png" style="max-height:35px; transform:translate(20px, -9px);" alt="Imagen"></router-link><!--<span style="color:white; font-size:20px;"> Clinic Match</span>--></div>
...
<router-link v-if="!isLoggedIn" tag="a" to="/login" class="loginbtn" href="">
   ...
   <router-link tag="div" to="/PanelUsuario" style="cursor:pointer; outline:none; padding:0px; margin-right:20px; color:black; display:inline-block; transform:translate(0px, 0px);"   v-if="isLoggedIn"><i class="fa fa-user" style="font-size:16px;  margin-right:5px;" aria-hidden="true"/>  {{usuario_datos.nombre}}</router-link>
