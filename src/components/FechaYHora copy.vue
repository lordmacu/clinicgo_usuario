...
<div>
  ...
  <div style="text-align:center; margin:auto; max-width:1200px; ">
                ...
                <div style="text-align:left;" v-if="!cargando_disponibles">
                  ...
                  <div v-for="h in horasDisponiblesC" v-if="horasDisponiblesC.length >0 && h.desde < 720" class="horas" v-on:click="scrolltoend(); tiSelected = h.desde; tfSelected= h.hasta; categorias_selected = h.categorias;  $gtag.event('fechayhora-seleccionar_hora', {'event_category': 'agendar', 'event_label': h.desde, 'value': 1} );" :style="{background: h.desde == tiSelected && h.hasta == tfSelected ? 'rgb(20, 134, 231)': '', color: h.desde == tiSelected && h.hasta == tfSelected ? 'white': ''}" >{{timeConvert(h.desde)}} - {{timeConvert(h.hasta)}} </div>
      ...
      <router-link :to="{name:'Home'}" tag="button" style="margin-top:10px; float:left;" class="btn btn-secondary"><b-icon style="transform:translateY(2px);" icon="arrow-left-circle"/> Volver</router-link>
              ...
              <button id="botoncontinuar" v-on:click="setData(selectedDia, selectedMes, selectedAño, tiSelected, tfSelected)" style="margin-top:10px; float:right;" v-bind:class="{'btn-success': tiSelected != -1, 'btn-danger': tiSelected == -1}" class="btn">Continuar a datos <b-icon style="transform:translateY(2px);" icon="arrow-right-circle"/></button>
...
export default {
  ...
  methods: {
    ...mapActions(['clickearExamenAction', 'setFechaYHoraAction']),
    scrolltoend(){
      window.scrollTo(0,document.body.scrollHeight);
    },
    setData(dia, mes, año, inicio, termino){
      if (this.selectedDia != -1 && this.selectedMes != -1 && this.selectedAño != -1 && this.tiSelected != -1 && this.tfSelected != -1){
      
        this.setFechaYHoraAction({dia:dia, mes:mes, año:año, inicio:inicio, termino:termino})
  ...
  computed: {
    ...mapGetters(['isLoggedIn','usuario_datos', 'comuna', 'comunas', 'examenes', 'examenes_agregados', 'dia'])
